...
$(document).ready(function () {
    ...
    $(document).on('click', '[role="modal-remote-bulk"]', function (event) {
        ...
        if (selectedIds.length == 0) {
            ...
            modal.show();
            modal.setTitle('Не выбрано');
            modal.setContent('Не выбрано ни одного элемента');
            modal.addFooterButton("Закрыть", 'btn btn-default', 'data-bs-dismiss=modal', function (button, event) {
                this.hide();
            });
        } else {
            // Open modal
